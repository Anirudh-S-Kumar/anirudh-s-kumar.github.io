<script lang="ts">
	...
	export let answer_list: string[] = [];
	export let link_list: string[] = [];
...
<!-- <div class="mb-10">
	{#if is_list}
		<span class="text-[#E5C07A] ">[</span>
		{#each answer_list as ans, i (i)}
            <span class="text-[#E5C07A]">"</span>
			{#if is_link}
				<a class="text-[#62AEEF]" href={link_list[i]}>{ans}</a>
			{:else}
				<span class="text-[#E5C07A]">{ans}</span>
			{/if}
            <span class="text-[#E5C07A]">"</span>
			<span class="text-[#E5C07A]">{i === answer_list.length - 1 ? '' : ', '}</span>
		{/each}
		<span class="text-[#E5C07A]">]</span>
	{:else if is_link}
		<a class="text-[#62AEEF]" href={link}>"{answer}"</a>
	{:else}
		<span class="text-[#E5C07A]">"{answer}"</span>
	{/if}
</div> -->
